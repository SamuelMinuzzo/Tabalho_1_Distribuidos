...
static inline long long bmh_com_prefetch_paralelo(const unsigned char * __restrict__ texto, long long tamanho, const unsigned char * __restrict__ palavra, size_t tamanho_palavra, const size_t * __restrict__ tabela, size_t salto_match) {
    if (tamanho_palavra <= 1 || tamanho < (long long)tamanho_palavra || tamanho < MIN_PARALLEL_BYTES) {
        return bmh_com_prefetch(texto, tamanho, palavra, tamanho_palavra, tabela, salto_match);
    }
...
#pragma omp parallel for reduction(+:count) schedule(static)
    for (int t = 0; t < num_threads; t++) {
        ...
        if (tam_local > 0) {
            count += bmh_com_prefetch(texto + ini, tam_local, palavra, tamanho_palavra, tabela, salto_match);
        }
    }
...
long long conta_palavras_bmh(const char *arq, const unsigned char *palavra, int tamanho_palavra) {
    ...
    while (1) {
        ...
        count += bmh_com_prefetch_paralelo(buffer, tamanho_valido, palavra, m, tabela, salto_match);
